...
| 8 | Downloaded Postman collection gets a real response | ✅ | `GET /v1/agents/{id}/postman` builds a v2.1 collection with a fresh `forge_…` API token in its `token` variable; Bearer tokens resolve to their owner; verified: the token from the download → `POST /invoke` → 202 with a real answer; another company's agent id → 404 | `app/api/routers/public_api.py` · `app/core/security.py` (API tokens) · `platform.api_tokens` · `tests/graded/test_check_08_postman.py` |
...
| `platform.api_tokens` | `token_hash` / `prefix` | only a sha256 of the token is stored; the plaintext is shown once, at creation; `prefix` is what the list shows |
