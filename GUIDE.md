...
| 4 Test it | ✅ except scores + API tab | Agent page, graph from config, tools table, **Playground** (chat; a write tool pauses the run and shows Approve / Reject inline; survives an API restart; 👍/👎), **Runs** tab (status, latency, feedback, outcome). **No scores yet, no API tab.** | `app/api/routers/runs.py` · `app/models/tenant.py` (`Run`) · `app/runtime/compiler.py` · `app/runtime/guarded_tool.py` · `web/src/pages/Playground.jsx` · `web/src/pages/AgentDetail.jsx` — pending: `app/scoring/`, API tab |
